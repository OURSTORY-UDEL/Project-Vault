...
import { ExternalLink, Code2, Eye, X, ArrowRight, Terminal, Sparkles, FileText } from "lucide-react"; 
...
const ProjectModal = ({ project, onClose, onUpdateProject }) => {
  ...
  const handleSaveCode = async (newCode) => {
    setIsSaving(true);
    const { error } = await supabase.from('projects').update({ code_snippet: newCode }).eq('id', project.id);
    if (!error) {
        ...
        <div className="flex-1 overflow-hidden relative bg-[#050505]">
          {viewMode === "preview" ? (
            <div className="w-full h-full flex items-center justify-center relative group p-10">
              <img src={project.preview_image || "https://placehold.co/800x600/1e1e1e/FFF?text=No+Preview"} alt="Preview" className="max-w-full max-h-full object-contain rounded-xl shadow-2xl border border-white/5" />
            ...
            <CodeEditor initialCode={project.code_snippet || "// Kosong"} onSave={handleSaveCode} isSaving={isSaving}/>
          )}
        </div>
...
// --- Main Page (ADA UPDATE TOMBOL NOTES) ---
    ...
    <div className="min-h-screen bg-black text-white font-sans selection:bg-purple-500/30 overflow-x-hidden relative">
      ...
      <header className="relative z-10 pt-20 pb-16 px-6 max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-end gap-8 border-b border-white/5">
        <div className="space-y-4">
          ...
          <h1 className="text-6xl md:text-8xl font-bold tracking-tighter bg-clip-text text-transparent bg-gradient-to-r from-white via-white to-neutral-600">The Vault.</h1>
        ...
        <div className="flex gap-4">
          {/* Tombol ke NOTES (BARU) */}
          <button 
            onClick={() => router.push('/notes')}
            className="group flex items-center gap-3 px-6 py-3 bg-neutral-900 border border-white/10 text-white font-bold rounded-full hover:bg-neutral-800 transition-all"
          >
      ...
      <main className="relative z-10 py-12 px-6 max-w-7xl mx-auto">
        {loading && <div className="grid grid-cols-1 md:grid-cols-3 gap-8">{[1,2,3].map(i => <div key={i} className="h-64 bg-white/5 rounded-3xl animate-pulse"/>)}</div>}
        {!loading && projects.length === 0 && (
          <div className="text-center py-32 border border-dashed border-white/10 rounded-3xl bg-white/5">
            ...
            <p className="text-neutral-600 mt-2">Masuk ke Dashboard untuk mengisi vault.</p>
          </div>
        )}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 auto-rows-[350px]">
          {projects.map((project, i) => (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} key={project.id} onClick={() => setSelectedProject(project)} className={`group relative rounded-[2rem] overflow-hidden cursor-pointer border border-white/10 bg-[#0a0a0a] ${project.size === 'large' ? 'md:col-span-2' : ''}`}>
              <div className="absolute inset-0 z-0">
                 <img src={project.preview_image || "https://placehold.co/600x400/101010/FFF"} alt={project.title} className="w-full h-full object-cover transition duration-700 group-hover:scale-110 opacity-60 group-hover:opacity-40"/>
                ...
                <div className="flex justify-between items-start">
                   <div className="flex flex-wrap gap-2">
                      {project.tags?.slice(0, 3).map(tag => (
                        <span key={tag} className="text-[10px] font-bold uppercase tracking-wider bg-white/10 backdrop-blur-md px-2 py-1 rounded border border-white/5 text-neutral-300">{tag}</span>
                      ))}
                   </div>
                   <div className="w-10 h-10 rounded-full bg-white text-black flex items-center justify-center transform -rotate-45 group-hover:rotate-0 transition duration-300 opacity-0 group-hover:opacity-100 scale-50 group-hover:scale-100"><ArrowRight size={20} /></div>
                </div>
                <div className="transform translate-y-4 group-hover:translate-y-0 transition duration-300">
                   <h3 className="text-3xl font-bold text-white mb-2 leading-tight group-hover:text-purple-300 transition-colors">{project.title}</h3>
                   <p className="text-neutral-400 text-sm line-clamp-2 opacity-0 group-hover:opacity-100 transition duration-500 delay-100">{project.description}</p>
                </div>
              </div>
            </motion.div>
          ))}
      ...
      <footer className="relative z-10 py-8 text-center text-neutral-600 text-xs font-mono"><p>PROJECT VAULT SYSTEM © {new Date().getFullYear()}</p></footer>
      <AnimatePresence>
        {selectedProject && <ProjectModal project={selectedProject} onClose={() => setSelectedProject(null)} onUpdateProject={handleUpdateProject} />}
      </AnimatePresence>
    </div>
  );
}